...
bits_p_sym = 1; % number of bits per symbol
nbr_sym_data = 2^16;
...
rate_error_1it_soft = [];
rate_error_5it_soft  = [];
rate_error_10it_soft  = [];
rate_error_20it_soft  = [];
...
H0 = generate_ldpc(K_H, N_H,0,1,5);
...
map = mapping(final_vect,bits_p_sym,'pam');
ma2= [];
for l = map'
    ma2 = [ma2,0,0,0,0,l,0,0,0,0,0];
end

map3 = mapping(vect,bits_p_sym,'pam');
ma3= [];
for i_uncoded = map3'
...
nb_point_BER = -1:2:15; % BER that will be explore to draw curves
...
for k = 1:nIterations
    ...
    Y = sqrt(N0 * 50E6) .* (randn(1,length(s))) ;  % Creating white gaussian noise
    Y_uncoded = sqrt(N0_uncoded * 50E6) .* (randn(1,length(s_uncoded))) ;  % Creating white gaussian noise
...
%______________________Soft decoding here_____________________________________________

    [bits_stream_1it_soft] = soft_decoding_log(r,H,var(Y),1);
    [bits_stream_5it_soft] = soft_decoding_log(r,H,var(Y),5);
    [bits_stream_10it_soft] = soft_decoding_log(r,H,var(Y),10);
    [bits_stream_20it_soft] = soft_decoding_log(r,H,var(Y),20);

%________________________demapping____________________________________________________

    demap_uncoded = demapping(r_uncoded,bits_p_sym,'pam');
    demap = demapping(r,bits_p_sym,'pam');
...
%_______________________decoding_____________________________________________________
    ...
    % [bits_stream_2it] = decoding(demap,H,2);
...
%____________________error computation_________________________________________________ 
    ...
    error_vct_5it_soft = final_vect' - bits_stream_5it_soft;
    error_vct_10it_soft = final_vect' - bits_stream_10it_soft;
    error_vct_20it_soft = final_vect' - bits_stream_20it_soft;

    nb_error = zeros(1,9);
    length_error = length(error_vct_uncoded);
    error_vct_uncoded = paddata(error_vct_uncoded,length(error_vct_1it_soft));
    % error_vct = [error_vct_uncoded';error_vct_1it;error_vct_2it;error_vct_5it;error_vct_10it];
    error_vct = [error_vct_uncoded';error_vct_1it_soft;error_vct_5it_soft;error_vct_10it_soft;error_vct_20it_soft;];
    % error_vct = [error_vct_uncoded';error_vct_1it;error_vct_2it;error_vct_5it;error_vct_10it;error_vct_1it_soft;error_vct_5it_soft;error_vct_10it_soft;error_vct_20it_soft;];
    ...
    rate_error_uncoded = [rate_error_uncoded,nb_error(1)/length_error];
    ...
    rate_error_5it_soft  = [rate_error_5it_soft ,nb_error(3)/length(error_vct_5it_soft )];
    rate_error_10it_soft  = [rate_error_10it_soft ,nb_error(4)/length(error_vct_10it_soft )];
    rate_error_20it_soft  = [rate_error_20it_soft ,nb_error(5)/length(error_vct_10it_soft )];
...
semilogy(nb_point_BER,rate_error_5it_soft)
semilogy(nb_point_BER,rate_error_10it_soft)
semilogy(nb_point_BER,rate_error_20it_soft)

legend('uncoded','soft 1it','soft 5it','soft 10it','soft 20it')
xlabel('SNR')
ylabel('BER')
