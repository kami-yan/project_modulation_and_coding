...
ppm_index = 1;
nb_avg = 10;
point_BER = -1.5:1:16; % BER that will be explore to draw curves
nIterations  = length(point_BER);


for time_shift_index = 1:length(time_shifts) 
...
for k = 1:nIterations
    progress = (time_shift_index-1)/length(time_shifts)+(avg-1)/nb_avg*1/length(time_shifts)+ 1/length(time_shifts)*1/nb_avg*k / nIterations  ;
...
legend(sprintf('time shift = %d s',time_shifts(1)/f_sampling),sprintf('time shift = %d s',time_shifts(2)/f_sampling),sprintf('time shift = %d s',time_shifts(3)/f_sampling),sprintf('time shift = %d s',time_shifts(4)/f_sampling),"Location","southwest")
title('BER with a time shift error 16QAM')
ylabel('BER')
xlabel('Eb/N0')
axis([min(point_BER) max(point_BER) 10^-5 1])
hold off
close(h);
